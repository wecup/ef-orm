...
final class BindVariableTool {
	 ...
	 * @author jiyi
	 */
	public static List<Object> setVariables(ConditionQuery da, SqlType operType, List<Field> writeFields, List<BindVariableDescription> whereFiels, 
			BindVariableContext context) throws SQLException {
		int count = 0;
		// 插入、更新值绑定
		if (writeFields != null) {
			Assert.isTrue(da instanceof Query<?>);
