 ...
 * 
 * <h3>什么是元模型</h3>
 * 元模型<meta-model>是一张数据库表的结构在ORM中的描述。<br>
 * 元模型是一个Java对象，这个对象中存放了数据库表的各种字段、类型等结构定义。
 * 我们可以通过元模型来指定一张表和这张表对应的Java映射类型。
 *
 *  <h3>元模型的获得</h3>
 *  一般来说，我们会使用一个类对应数据库中的一张表。比如类 jef.orm.test.Person对应数据库中的 PERSON表。
 *  我们可以用以下方法来获得这个表的元模型。<pre><tt>
 *  ITableMetadata metaModel=MetaHolder.getMeta(jef.orm.test.Person.class)
 *  </tt></pre>
 *  
 *  <h3>元模型的种类</h3>
 *  在EF-ORM中，有三种不同的元模型。 {@link #getType()}
 *  <li>{@link EntityType#NATIVE}<br>基础映射，一个class对应一张表。<br>这个class必须实现jef.database.IQueryableEntity接口。这也是EF-ORM中标准的O-R映射方式<br></li>
 *  <li>{@link EntityType#TUPLE}<br>动态的映射，表没有对应的java类对应，而是用一个类似于Map的结构（{@link jef.database.VarObject}）来对应。</li>
 *  <li>{@link EntityType#POJO}<br>扩展功能，为了支持一些简单的单表CRUD操作，<br>EF-ORM也可以让一些POJO类映射到数据库表，提供基本的对象操作功能。</li>
 * 
 * @author Jiyi
...
public interface  ITableMetadata {
	...
	/**
	 * 得到此表对应的java class
	 * @return 对应的java class，当metadata为POJO类型时，此处返回PoJoWrapper，为动态表类型时，返回VarObject
	 */
	...
	/**
	 * 得到schema名称。
	 *  ORM允许用户使用Annotation @Table(schema="S1") 添加在实体类上，指定该实体操作位于特定的schema上。<br>
	 ...
	 * @return 返回表名，如果实体绑定了schema，并且withSchema为true,那么返回形如 <code>schema.table</code>的名称
	 */
	...
	/**
	 * 根据数据库列名（小写的）获得field对象
	 * 注意传入的列名必须保持小写。
	 * @param columnInLowerCase 数据库列名的小写
	 * @return FIeld对象(字段元模型)
	 */
	 ...
	 * @return 外键关系
	 */
	public Reference findPath(ITableMetadata target);
	 ...
	 */
	public Reference findDistinctPath(ITableMetadata target) throws IllegalArgumentException;
	
	////////////////////////附加功能////////////////////
	/**
	 * 返回所有的元模型字段和类型。这些字段的顺序会进行调整，CLob和BLob将会被放在最后。
	 * 这些字段的顺序一旦确定那么就是固定的
	 * @return
	 */
	...
	/**
	 * 获得每个字段上，最小单位的分表函数。
	 * 也就是说，其实一个字段上可以对应多个Key，例如有一个Date birthDay
	 * 然后KeyA function=YEAR
	 *    KeyB function=MONTH
