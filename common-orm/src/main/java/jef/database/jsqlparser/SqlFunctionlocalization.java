...
//			log.warn("Can't find user script file for user function "+ sf);
	...
	/**
	 * 只有PG和MYSQL是支持interval语法的，但是两者的语法也有区别。
	 * Oracle不支持Interval，但是支持以1为一天的小数运算，如 1/86400=1秒 1/1440=1分 1/24的小时。 oracle可以用整数运算来实现天数的加减。 oracle提供了add_months来实现月数的加减, 可以用add_months 12 /-12来实现加减年份
	 */
	@Override
	public void visit(Interval interval) {
		...
		if(parent instanceof BinaryExpression){
			...
			Function func=((ExpressionList) parent).getParent();
			profile.processIntervalExpression(func,interval);
		}
	}
