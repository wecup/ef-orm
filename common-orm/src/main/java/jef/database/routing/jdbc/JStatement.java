...
public class JStatement implements java.sql.Statement {
	 ...
	 * 
	 */
	protected ResultSet executeQueryInternal(String sql,Collection<ParameterContext> params) throws SQLException {
		...
		if(resultSetType>0)
			executor.setResultSetType(resultSetType);
		if(resultSetConcurrency>0)
			executor.setResultSetConcurrency(resultSetConcurrency);
		if(resultSetHoldability>0)
			executor.setResultSetHoldability(resultSetHoldability);
		if(params!=null)
	...
	protected int[] executeBatchInternal(String sql, int autoGeneratedKeys, int[] columnIndexes, String[] columnNames,List<Collection<ParameterContext>> params) throws SQLException {
		jef.database.jsqlparser.visitor.Statement st = parse(sql);
		SQLExecutor se;
		...
		this.updateReturn= se.executeBatch(autoGeneratedKeys, columnIndexes, columnNames,params);
		return updateReturn.getBatchResult();
		
	}
	protected int executeUpdateInternal(String sql, int autoGeneratedKeys, int[] columnIndexes, String[] columnNames,Collection<ParameterContext> params) throws SQLException {
		jef.database.jsqlparser.visitor.Statement st = parse(sql);
		SQLExecutor se;
		if (st == null) { //无法解析，直接运行
			se=new SimpleSQLExecutor((OperateTarget) conn.get().getSqlTemplate(null),sql);
		...
		if(params!=null)
			se.setParams(params);
		this.updateReturn= se.executeUpdate(autoGeneratedKeys,columnIndexes,columnNames);
		return updateReturn.getAffectedRows();
	}
