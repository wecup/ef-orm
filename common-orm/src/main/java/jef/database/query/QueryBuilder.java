 ...
 * 
 * JEF提供的查询方式包括
 * 1. 简单对象查询（提供各种单表的、批的、级联的数据库操作）<br/>
 * 2、Criteria API（还可以提供基于多表的、Or/Not、Join的、exists/not exists、Union的、group/having（投影操作）等各种SQL）。<br/>基本上能涵盖标准SQL语法可以执行的各种操作<br/>
 * 3.NamedQuery/NativeQuery。 （可以实现各种DDL，还有数据库专有语法：如Oracle的分析函数、递归(connect by）等原生的数据库操作<br/>
...
public class QueryBuilder {
	...
	public static <T extends IQueryableEntity> Query<T> create(Class<T> clz){
		...
		Query<T> query= (Query<T>) d.getQuery();
	...
	/**
	 * 创建一个查询请求,当不赋任何条件的时候，这个查询带有一个默认条件为为 AllRecordsCondition.
	 * @param clz 要查询的表的元模型 {@linkplain ITableMetadata 什么是元模型}
	 * @return
	 */
	...
	/**
	 * 创建一个查询请求,当不赋任何条件的时候，这个查询带有一个默认条件为为 AllRecordsCondition.
	 * @param clz 要查询的表(动态表)的元模型，{@linkplain ITableMetadata 什么是元模型}
	 * @return
	 */
	...
	/**
	 * 产生相等条件 (=)
	 * @param field 表的字段 （也可以是函数表达式）
	 * @param value 条件的值，一般传入String,Number,Date等基本数据，也可传入Field对象、或者是SqlExpression等对象。
	...
	/**
	 * 得到一个Like %str%的条件
	 * @param field  field 表的字段（也可以是函数表达式）
	 * @param str 要匹配的字符串
	 * @return 产生形如 {@code field like '%str%' } 这样的条件，str中原来的的'%' '_'符号会被转义
	 */
	...
	/**
	 * 得到一个Like条件，参数为自定义的字符串。
	 * 例如<p>
	 * {@code like(field, "%123_456%")}<p>
	 *  相当于<p>
	 *  {@code WHERE field LIKE '%123_456%'  }<p>
	 *  
	 *  <h3>注意</h3>
	 *  这个方法可以自由定义复杂的匹配模板外，但是和matchxxx系列的方法相比，不会对字符串中的'%' '_'做转义。因此实际使用不当会有SQL注入风险。<p>
	 * @param field   field 表的字段（也可以是函数表达式）
	 * @param str 要匹配的字符串
	...
	/**
	 * 将多个普通查询拼为union查询。 要求每个查询返回的字段个数和字段类型必须一致
	 * @param clz UNION返回结果的类型
	 * @param queries queries 多个查询
	 * @return UnionQuery对象。
	 */
	 ...
	 * @param clz union all查询要返回的类型
	 * @param queries 构成union all的多个查询
	 * @return UnionQuery对象。
	 */
	...
	/**
	 * 创建内连接
	 * @param left 连接左边的查询。可以是{@link Query}或者{@link Join}对象
	 * @param right 右边的查询
	 * @param keys 连接路径，可省略。一般在编程中QB.on()方法来构造
	 * @return Join查询对象
	 * @see #on(Field, Field)
	 * @see #on(Field, Number)
	 * @see #on(Field, String)
	...
	public static Join innerJoin(JoinElement left,Query<?> right,JoinKey... keys){
		...
		if(join==null)join= JoinUtil.create(left, right, path,JoinType.INNER,true);
	...
	public static Join leftJoin(JoinElement left,Query<?> right,JoinKey... keys){
		...
		if(join==null)join = JoinUtil.create(left, right, path,JoinType.LEFT,true);
		Assert.notNull(join,"Join create failure on !");
		return join;
	}
	...
	public static Join rightJoin(JoinElement left,Query<?> right,JoinKey... keys){
		...
		if(join==null)join=JoinUtil.create(left, right, path,JoinType.RIGHT,true);
		Assert.notNull(join,"Join create failure on !");
		return join;
	}
	...
	/**
	 * 保留left查询中一对一关系或者多对一关系产生的外连接查询，然后再关联右侧查询对象(左外连接)。
	 * <h3>使用举例</h3>
	 * TODO documented.
	 * 
	 * @param left 连接左边的查询。是{@link Query}对象
	 * @param right 右边的查询
	 * @param keys 连接路径，可省略。一般在编程中QB.on()方法来构造
	 * @return Join查询对象
	 */
	...
	/**
	 * 保留left查询中一对一关系或者多对一关系产生的外连接查询，然后再关联右侧查询对象（右外连接）。
	 *  <h3>使用举例</h3>
	 * TODO documented.
	 * @param left 连接左边的查询。是{@link Query}对象
	 * @param right  右边的查询
	 * @param keys 连接路径。一般在编程中QB.on()方法来构造
	 * @return  Join查询对象
	 */
	...
	/**
	 * 保留left查询中一对一关系或者多对一关系产生的外连接查询，然后再关联右侧查询对象（全外连接）。
	 * <h3>使用举例</h3>
	 * TODO documented.
	 * @param left 连接左边的查询。是{@link Query}对象
	 * @param right 右边的查询
	...
	/**
	 * 创建一个列选择操作器（SelectItems），SelectItems提供了若干方法用于指定select语句中的选择列，包括distinct, group by, hvaing等语法支持。
	 * 使用此API可以构造出投影操作，如max() min() sum()等分组函数。
	 * @param query 查询
	 * @return 列选择操作器
	 * @see Selects
	 */
...
//	}
		if(query.getSelectItems()!=null && (query.getSelectItems() instanceof Selects))return (Selects)query.getSelectItems();
		List<QueryAlias> qs;
	 ...
	 * 创建连接键描述
	 * @param left 左边的字段
	 * @param value 常量string，即要求等于右侧的常量值
	 * @return
	 */
