...
import org.easyframe.fastjson.parser.DefaultJSONParser.ResolveTask;
import org.easyframe.fastjson.parser.Feature;
import org.easyframe.fastjson.parser.JSONLexer;
import org.easyframe.fastjson.parser.JSONLexerBase;
import org.easyframe.fastjson.parser.JSONToken;
import org.easyframe.fastjson.parser.ParseContext;
import org.easyframe.fastjson.parser.ParserConfig;
import org.easyframe.fastjson.parser.SymbolTable;
import org.easyframe.fastjson.parser.deserializer.ASMJavaBeanDeserializer.InnerJavaBeanDeserializer;
...
public class ASMDeserializerFactory implements Opcodes {
    ...
    public ObjectDeserializer createJavaBeanDeserializer(ParserConfig config, Class<?> clazz, Type type)
        ...
        cw.visit(V1_5, ACC_PUBLIC + ACC_SUPER, className,null, getType(ASMJavaBeanDeserializer.class), null);
        ...
        {
        	 for(FieldInfo fi:beanInfo.getFieldList()){
    ...
    void _deserialzeArrayMapping(ClassWriter cw, Context context) {
        ...
        for (int i = 0; i < fieldListSize; ++i) {
            ...
            if (fieldClass == byte.class //
                ...
                mw.visitVarInsn(ALOAD, context.var("lexer"));
                mw.visitLdcInsn(jef.accelerator.asm.Type.getType(getDesc(fieldClass)));
                ...
                if (itemClass == String.class) {
                    ...
                    mw.visitVarInsn(ALOAD, 1);
                    mw.visitLdcInsn(jef.accelerator.asm.Type.getType(getDesc(itemClass)));
    ...
    void _deserialze(ClassWriter cw, Context context) {
        ...
        mw.visitMethodInsn(INVOKEVIRTUAL, getType(DefaultJSONParser.class), "getContext",
                           "()Lorg/easyframe/fastjson/parser/ParseContext;");
        ...
        for (int i = 0; i < fieldListSize; ++i) {
            ...
            if (fieldClass == boolean.class) {
                ...
                if (itemClass == String.class) {
                    mw.visitLdcInsn(jef.accelerator.asm.Type.getType(getDesc(fieldClass))); // cast
    ...
    private void _loadAndSet(Context context, MethodVisitor mw, FieldInfo fieldInfo) {
        Class<?> fieldClass = fieldInfo.getFieldClass();
        Type fieldType = fieldInfo.getFieldType();
        if(fieldInfo.getMethod()==null && fieldInfo.accessor!=null){//push FieldAccessor into operand first.
        	mw.visitVarInsn(ALOAD, 0);//S3
        	mw.visitFieldInsn(GETFIELD, context.getClassName(), "_a_"+fieldInfo.getName(), getDesc(FieldAccessor.class));//S1
        }
        if (fieldClass == boolean.class) {
            mw.visitVarInsn(ALOAD, context.var("instance"));
    ...
    private void _set(Context context, MethodVisitor mw, FieldInfo fieldInfo) {
        ...
        } else if(fieldInfo.accessor!=null){
        	if(fieldInfo.getFieldClass().isPrimitive()){
        		ASMUtils.doWrap(mw, fieldInfo.getFieldClass());
        	}
        	mw.visitMethodInsn(INVOKEVIRTUAL, getType(FieldAccessor.class), "set", "(Ljava/lang/Object;Ljava/lang/Object;)V");//S1
        } else{
            mw.visitFieldInsn(PUTFIELD, getType(fieldInfo.getDeclaringClass()), fieldInfo.getField().getName(),
                              getDesc(fieldInfo.getFieldClass()));
        }
    ...
    private void _deserialze_list_obj(Context context, MethodVisitor mw, Label reset_, FieldInfo fieldInfo,
        ...
        mw.visitFieldInsn(GETFIELD, context.getClassName(), fieldInfo.getName() + "_asm_list_item_deser__",
                          getDesc(ObjectDeserializer.class));
        mw.visitVarInsn(ALOAD, 1);
        mw.visitLdcInsn(jef.accelerator.asm.Type.getType(getDesc(itemType)));
    ...
    private void _deserObject(Context context, MethodVisitor mw, FieldInfo fieldInfo, Class<?> fieldClass) {
        ...
        if (fieldInfo.getFieldType() instanceof Class) {
            mw.visitLdcInsn(jef.accelerator.asm.Type.getType(getDesc(fieldInfo.getFieldClass())));
        } else {
            mw.visitVarInsn(ALOAD, 0);
            mw.visitLdcInsn(fieldInfo.getName());
    ...
    public FieldDeserializer createStringFieldDeserializer(ParserConfig mapping, Class<?> clazz, FieldInfo fieldInfo)
        ...
        cw.visit(V1_5, ACC_PUBLIC + ACC_SUPER, className, null,getType(superClass), null);
    ...
    private void _init(ClassWriter cw, Context context) {
        for (int i = 0, size = context.getFieldInfoList().size(); i < size; ++i) {
            ...
            FieldVisitor fw = cw.visitField(ACC_PUBLIC, fieldInfo.getName() + "_asm_prefix__", "[C",null,null);
            fw.visitEnd();
        }
        ...
        for (int i = 0, size = context.getFieldInfoList().size(); i < size; ++i) {
            ...
            if (Collection.class.isAssignableFrom(fieldClass)) {
                FieldVisitor fw = cw.visitField(ACC_PUBLIC, fieldInfo.getName() + "_asm_list_item_deser__",
                                                getDesc(ObjectDeserializer.class),null,null);
                fw.visitEnd();
            } else {
        ...
        for(FieldInfo fi:context.getBeanInfo().getFieldList()){
        	if(fi.accessor!=null){//private field
        		FieldVisitor fw = cw.visitField(ACC_PUBLIC, "_a_"+fi.getName(), getDesc(FieldAccessor.class),null,null);
        		fw.visitEnd();		
        	}
        }
    }
    ...
    private void _createInstance(ClassWriter cw, Context context) {
        MethodVisitor mw = cw.visitMethod(ACC_PUBLIC, "createInstance", "(" + getDesc(DefaultJSONParser.class)
                                                                        + getDesc(Type.class) + ")Ljava/lang/Object;", null,null);
