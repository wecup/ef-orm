...
import jef.rowset.internal.CachedRowSetReader;
import jef.rowset.internal.CachedRowSetWriter;
...
//        }
        ...
        rowSetReader = new CachedRowSetReader();//(CachedRowSetReader)provider.getRowSetReader();
        rowSetWriter = new CachedRowSetWriter();//(CachedRowSetWriter)provider.getRowSetWriter();
     ...
     * optimistic provider <code>jef.rowset.providers.RIOptimisticProvider</code>.
     * <p>
            ...
            for ( i = 1; i <= numCols; i++) {
                ...
                if (map == null) {
                	try{
                		...
                		//PG has bug on handle type MONEY, it attempts to convert money to double, even if it is a string with currency unit. 
    ...
    private void initMetaData(RowSetMetaDataImpl md, ResultSetMetaData rsmd) throws SQLException {
        ...
        for (int col=1; col <= numCols; col++) {
            ...
            if(rsmd.isAutoIncrement(col))
                updateOnInsert = true;
            md.setCaseSensitive(col, isCaseSensitive); //fix the MySQL performance issue:http://blog.csdn.net/match_ding/article/details/11078553
